...
#define NINE 0b01101111
    ...
    u8 u8_localar[10]={ZERO,ONE,TWO,THREE,FOUR,FIVE,SIX,SEVEN,EIGHT, NINE};
...
int main(void)
{
	...
	MEXTI_voidsetCallBack(EXTI0, button_pressed);
    MEXTI_voidEnable(EXTI0);
    u8 local_u8_DATA;
    while (1)
    {
		 if(flag==1) //send if interrupt happens
		 {
			MDIO_voidSetPinValue(PORTA, PIN0, PIN_LOW); // ss``
			local_u8_DATA = MSPI_u8_Getdata(5);
